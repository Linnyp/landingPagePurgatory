    ...
    <main className="overflow-hidden bg-sand-50 pt-20 text-sand-950">
      <section className="relative border-b-4 border-sand-950 py-20 md:py-28">
        ...
        <div className="relative mx-auto grid w-full max-w-[1200px] items-center gap-14 px-6 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            <p className="mb-5 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">How it works</p>
            <h1 className="max-w-[680px] font-brand text-[clamp(3rem,6vw,5.75rem)] font-black uppercase leading-[0.91] tracking-[-0.055em]">
              One team. One platform. <span className="text-clay-500">One monthly price.</span>
            </h1>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-5">
              ...
              <a href="#consult" className="inline-flex items-center gap-2 font-brand text-sm font-bold uppercase tracking-[0.06em] text-sand-950 underline decoration-clay-500 decoration-2 underline-offset-8 hover:text-clay-600">
          ...
          <div className="relative mx-auto w-full max-w-[470px]">
            <div className="absolute inset-5 rounded-[44px] bg-clay-500" aria-hidden="true" />
            <div className="relative overflow-hidden rounded-[36px] border-4 border-sand-950 bg-sand-50 p-3 shadow-[10px_10px_0_0_#1f1b16]">
              <div className="flex items-center justify-between border-b-2 border-sand-200 px-3 py-3 font-brand text-xs font-bold uppercase tracking-[0.09em]">
                <span>Customer inbox</span><span className="h-2.5 w-2.5 rounded-full bg-clay-500" />
              </div>
              <div className="space-y-4 p-5">
                ...
                <div className="ml-auto max-w-[82%] rounded-2xl rounded-tr-sm bg-clay-500 p-4 text-sm font-medium leading-relaxed text-sand-50">Thanks for reaching out. We&apos;ll get you on the schedule.</div>
                <div className="flex items-center gap-3 rounded-2xl border-2 border-sand-200 p-4">
                  <FiCalendar className="shrink-0 text-clay-500" size={22} aria-hidden="true" />
      ...
      <section className="border-b-4 border-sand-950 bg-sand-100 bg-grid-pattern py-24 md:py-32">
        <div className="mx-auto w-full max-w-[1200px] px-6">
          <div className="mb-14 ml-auto max-w-[690px] text-right">
            <p className="mb-4 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">The process</p>
            <h2 className="font-brand text-[clamp(2.5rem,5vw,4.5rem)] font-black uppercase leading-[0.92] tracking-[-0.055em]">A working system in <span className="text-clay-500">three steps.</span></h2>
          </div>
          <div className="grid grid-cols-1 border-t-4 border-sand-950 lg:grid-cols-3">
            {steps.map((step, index) => (
              <article key={step.number} className={`bg-sand-50 px-8 py-10 md:px-10 md:py-12 ${index > 0 ? "border-t-2 border-sand-950 lg:border-t-0 lg:border-l-2" : ""}`}>
                <p className="font-brand text-6xl font-black leading-none tracking-[-0.06em] text-clay-500">{step.number}</p>
      ...
      <section className="py-24 md:py-32">
        <div className="mx-auto w-full max-w-[1200px] px-6">
          <div className="max-w-[720px]">
            <p className="mb-4 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">The foundation</p>
          ...
          <div className="mt-14 grid gap-x-8 gap-y-5 md:grid-cols-2 lg:grid-cols-3">
            {foundationItems.map(({ icon: Icon, title, body }) => (
              <article key={title} className="rounded-2xl border-2 border-sand-200 bg-sand-50 p-6 transition-colors hover:border-clay-500">
                <Icon size={25} className="text-clay-500" aria-hidden="true" />
      ...
      <section className="bg-sand-100 py-24 md:py-32">
        <div className="mx-auto grid w-full max-w-[1200px] gap-14 px-6 lg:grid-cols-[5fr_7fr] lg:gap-0">
          <div className="lg:border-r-2 lg:border-sand-300 lg:pr-12">
            <p className="mb-4 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Your side</p>
            ...
            <div className="mt-9 grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
              {[
                ...
                return <article key={title as string} className="rounded-2xl bg-sand-50 p-5 shadow-sm"><CardIcon size={22} className="text-clay-500" aria-hidden="true" /><h3 className="mt-4 font-brand text-sm font-extrabold uppercase">{title as string}</h3><p className="mt-2 text-sm leading-relaxed text-sand-700">{body as string}</p></article>;
              })}
            </div>
          ...
          <div className="lg:pl-14">
            <p className="mb-4 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Our side</p>
            ...
            <ul className="mt-9 space-y-4" aria-label="What KeyLime manages">
              {managedWork.map((item) => <li key={item} className="flex gap-3 text-[15px] leading-relaxed text-sand-700"><FiActivity className="mt-1 shrink-0 text-clay-500" aria-hidden="true" /><span>{item}</span></li>)}
            </ul>
            <blockquote className="mt-10 border-l-4 border-clay-500 pl-5 font-brand text-xl font-bold leading-snug tracking-[-0.03em]">When something needs a human, you talk to a human. When it doesn&apos;t, the system handles it.</blockquote>
          </div>
        </div>
      </section>
      ...
      <section id="consult" className="bg-sand-900 py-24 text-sand-50 md:py-32">
        <div className="mx-auto w-full max-w-[1200px] px-6">
          <div className="max-w-[710px]">
            <p className="mb-4 font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">What next</p>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-2">
            ...
            <article className="rounded-3xl border border-clay-500/50 bg-sand-800 p-8 md:p-10"><h3 className="font-brand text-2xl font-black uppercase tracking-[-0.04em]">Not sure which one fits?</h3><p className="mt-4 max-w-[440px] leading-[1.65] text-sand-50/70">Free 30-minute consult. No pitch deck. We&apos;ll tell you which system fits and what it would take to get there.</p><div className="mt-8"><a href="https://calendly.com/keylime-marketing/discovery-call" className="inline-flex min-h-[50px] items-center justify-center gap-2 rounded-full bg-clay-500 px-6 py-3 text-sm font-bold uppercase tracking-[0.06em] text-sand-50 no-underline transition-transform hover:-translate-y-0.5">Book Free Consult <FiArrowRight aria-hidden="true" /></a></div></article>
          </div>
          <p className="mt-9 text-center text-sm text-sand-50/65">Still scoping the problem? <Link href="/calculators/missed-call-revenue" className="font-bold text-sand-50 underline decoration-clay-500 decoration-2 underline-offset-4">Calculate my missed-call revenue →</Link></p>
        </div>
      </section>
    </main>
  );
