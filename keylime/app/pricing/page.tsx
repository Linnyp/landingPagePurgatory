...
function CalendlyButton({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <a
      href={CALENDLY_URL}
      className={`inline-flex min-h-[50px] items-center justify-center gap-2 rounded-full bg-clay-500 px-6 py-3 text-sm font-bold uppercase tracking-[0.06em] text-sand-50 no-underline transition-transform duration-200 hover:-translate-y-0.5 hover:bg-clay-700 ${className}`}
    >
    ...
    <main className="overflow-hidden bg-sand-50 pt-20 text-sand-950">
      <section className="relative border-b-4 border-sand-950 py-20 md:py-28">
        ...
        <div className="relative mx-auto max-w-[1200px] px-6 text-center">
          <p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Simple, published pricing</p>
          <h1 className="mx-auto mt-5 max-w-[930px] font-brand text-[clamp(3rem,7vw,6rem)] font-black uppercase leading-[0.9] tracking-[-0.06em]">Three systems. <span className="text-clay-500">One monthly price.</span></h1>
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-5"><CalendlyButton>Book Free Consult</CalendlyButton><Link href="/test-calculator" className="inline-flex items-center gap-2 font-brand text-sm font-bold uppercase tracking-[0.06em] text-sand-950 underline decoration-clay-500 decoration-2 underline-offset-8 hover:text-clay-600">Calculate my missed-call revenue <FiArrowRight aria-hidden="true" /></Link></div>
          <p className="mt-8 font-brand text-sm font-bold uppercase tracking-[0.04em] text-sand-700">$200 one-time setup <span className="px-1.5 text-clay-500">·</span> Month-to-month <span className="px-1.5 text-clay-500">·</span> Live in 4 to 8 weeks</p>
        </div>
      </section>

      <section className="bg-sand-100 py-24 md:py-32" aria-labelledby="systems-heading">
        <div className="mx-auto max-w-[1200px] px-6">
          <div className="mx-auto mb-16 max-w-[700px] text-center"><p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Choose your system</p><h2 id="systems-heading" className="mt-4 font-brand text-[clamp(2.4rem,5vw,4.4rem)] font-black uppercase leading-[0.93] tracking-[-0.055em]">Built for where the business is now.</h2></div>
          <div className="grid items-stretch gap-6 lg:grid-cols-3">
            {systems.map((system) => (
              <article key={system.name} className={`relative flex flex-col rounded-3xl p-8 md:p-9 ${system.featured ? "border-4 border-sand-950 bg-sand-950 text-sand-50 shadow-[0_16px_0_0_#c2552d]" : "border-2 border-sand-200 bg-sand-50 text-sand-950"}`}>
                {system.featured ? <span className="absolute -top-4 left-7 rounded-full bg-clay-500 px-4 py-2 font-brand text-[10px] font-bold uppercase tracking-[0.12em] text-sand-50">Most operators start here</span> : null}
                <h3 className="font-brand text-2xl font-black uppercase tracking-[-0.04em]">{system.name}</h3>
                <p className={`mt-2 font-brand text-[42px] font-black leading-none tracking-[-0.06em] ${system.featured ? "text-clay-500" : "text-clay-500"}`}>{system.price}<span className={`ml-1 text-base font-semibold tracking-normal ${system.featured ? "text-sand-50/65" : "text-sand-700"}`}>/mo</span></p>
                ...
                <ul className={`mt-7 space-y-3 border-t pt-7 text-sm leading-relaxed ${system.featured ? "border-sand-50/20" : "border-sand-200"}`}>
                  {system.features.map((feature) => <li key={feature} className="flex gap-3"><FiCheck className="mt-1 shrink-0 text-clay-500" aria-hidden="true" /><span>{feature}</span></li>)}
                </ul>
      ...
      <section className="py-24 md:py-32" aria-labelledby="terms-heading">
        <div className="mx-auto grid max-w-[1200px] gap-12 px-6 lg:grid-cols-[5fr_7fr] lg:items-end">
          <div><p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Clear from the start</p><h2 id="terms-heading" className="mt-4 font-brand text-[clamp(2.4rem,4.5vw,4.25rem)] font-black uppercase leading-[0.93] tracking-[-0.055em]">Setup is $200. The rest is month-to-month.</h2></div>
      ...
      <section className="py-24 md:py-32" aria-labelledby="custom-work-heading"><div className="mx-auto grid max-w-[1200px] gap-8 px-6 lg:grid-cols-[7fr_5fr]"><div><p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">More when you need it</p><h2 id="custom-work-heading" className="mt-4 font-brand text-[clamp(2.4rem,4.6vw,4.25rem)] font-black uppercase leading-[0.93] tracking-[-0.055em]">Need something beyond your system?</h2><p className="mt-6 max-w-[650px] text-[17px] leading-[1.7] text-sand-700">Add-ons are for individual capabilities you need now but do not need to bundle into a higher system yet. Bigger work gets a real scope and a real quote before you commit.</p></div><div className="rounded-3xl bg-sand-100 p-8 md:p-10"><h3 className="font-brand text-lg font-extrabold uppercase">Published add-ons</h3><p className="mt-3 text-sm leading-[1.7] text-sand-700">Voice Agents · Lead Nurture · Rewards · Database Reactivation · Google Business Profile management on lower systems · additional automations and chatbots.</p><h3 className="mt-8 font-brand text-lg font-extrabold uppercase">Custom-scoped work</h3><p className="mt-3 text-sm leading-[1.7] text-sand-700">Full SEO programs · Google and Meta ads management · custom website projects · ongoing content · large workflow builds.</p><Link href="/contact" className="mt-8 inline-flex items-center gap-2 font-brand text-sm font-bold uppercase tracking-[0.06em] text-sand-950 underline decoration-clay-500 decoration-2 underline-offset-8 hover:text-clay-600">Talk to us about custom work <FiArrowRight aria-hidden="true" /></Link></div></div></section>

      <section className="bg-sand-100 py-24 md:py-32" aria-labelledby="pricing-faq-heading"><div className="mx-auto max-w-[920px] px-6"><p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">Pricing FAQ</p><h2 id="pricing-faq-heading" className="mt-4 font-brand text-[clamp(2.4rem,4.6vw,4.25rem)] font-black uppercase leading-[0.93] tracking-[-0.055em]">The questions people ask before they pick.</h2><div className="mt-12 border-y-2 border-sand-950">{faqs.map((faq) => <details key={faq.question} className="group border-b-2 border-sand-950 last:border-b-0"><summary className="flex cursor-pointer list-none items-center justify-between gap-6 py-6 font-brand text-[15px] font-bold uppercase leading-[1.4] tracking-[0.01em] marker:content-none"><span>{faq.question}</span><span className="text-2xl text-clay-500 transition-transform group-open:rotate-45" aria-hidden="true">+</span></summary><p className="max-w-[720px] pb-7 text-[15px] leading-[1.75] text-sand-700">{faq.answer}</p></details>)}</div></div></section>

      <section className="bg-sand-900 py-24 text-sand-50 md:py-32"><div className="mx-auto max-w-[960px] px-6 text-center"><p className="font-brand text-xs font-bold uppercase tracking-[0.18em] text-clay-500">One clear next step</p><h2 className="mx-auto mt-4 max-w-[790px] font-brand text-[clamp(2.5rem,5.5vw,4.8rem)] font-black uppercase leading-[0.92] tracking-[-0.06em]">Not sure which system fits? We&apos;ll help you pick.</h2><p className="mx-auto mt-7 max-w-[640px] text-[17px] leading-[1.7] text-sand-50/70">Book a free 30-minute consult. No pitch deck. We&apos;ll look at where leads are falling through, what you already use, and the system that makes sense from there.</p><div className="mt-10 flex flex-wrap justify-center gap-5"><CalendlyButton>Book Free Consult</CalendlyButton><Link href="/test-calculator" className="inline-flex min-h-[50px] items-center justify-center gap-2 rounded-full border-2 border-sand-50/60 px-6 py-3 text-sm font-bold uppercase tracking-[0.06em] text-sand-50 no-underline transition-colors hover:border-clay-500 hover:text-clay-500">Calculate my missed-call revenue <FiArrowRight aria-hidden="true" /></Link></div><p className="mt-7 text-sm text-sand-50/55"><FiMessageCircle className="mr-2 inline text-clay-500" aria-hidden="true" />No pressure. Just a clear answer on what fits.</p></div></section>
    </main>
  );
}
