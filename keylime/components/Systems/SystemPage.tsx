...
export function SystemPage({ system }: { system: SystemKey }) {
  ...
  return <main className="overflow-hidden bg-sand-50 pt-20 text-sand-950">
    <section className="relative isolate overflow-hidden border-b-4 border-sand-950 py-20 md:py-28"><ParallaxBackground image="/background/keylimeLong.jpg" /><HeroGlow className="-left-32 -top-28 h-[460px] w-[460px]" /><HeroGlow className="-right-24 -bottom-32 h-[560px] w-[560px]" /><div className="relative z-10 mx-auto grid max-w-[1200px] gap-10 px-6 lg:grid-cols-[7fr_5fr] lg:items-end"><div><p className=" font-brand text-xs font-bold uppercase tracking-[.18em] text-clay-500">{content.eyebrow}</p><h1 className="mt-4 font-brand text-[clamp(3rem,6vw,5.7rem)] font-black uppercase leading-[.9] tracking-[-.06em]">{content.heading}</h1><p className="mt-7 max-w-[650px] text-[17px] leading-[1.7] text-sand-700">{content.intro}</p></div><div className="rounded-3xl bg-sand-950 p-8 text-sand-50 shadow-[10px_10px_0_0_#c2552d]"><p className="font-brand text-sm font-bold uppercase tracking-[.12em] text-clay-500">{content.name}</p><p className="mt-3 font-brand text-6xl font-black tracking-[-.06em]">{content.price}<span className="text-xl text-sand-50/65">/mo</span></p><p className="mt-3 text-sm leading-relaxed text-sand-50/65">$200 one-time setup · Month-to-month</p><a href={CALENDLY_URL} className="mt-7 inline-flex min-h-[50px] w-full items-center justify-center gap-2 rounded-full bg-clay-500 px-6 py-3 text-sm font-bold uppercase tracking-[.06em] text-sand-50 no-underline hover:bg-clay-700">Book Free Consult <FiArrowRight aria-hidden="true" /></a></div></div></section>
    <section className="bg-sand-100 py-7"><div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-4 px-6"><p className="font-brand text-sm font-bold uppercase tracking-[-.02em]">Not sure if {content.name} is right?</p><Link href="/calculators/missed-call-revenue" className="font-brand text-sm font-bold uppercase tracking-[.04em] text-sand-950 underline decoration-clay-500 decoration-2 underline-offset-4">See what missed calls are costing you →</Link></div></section>
    <WhoItFitsSection personaTitle={content.personaTitle} persona={content.persona} bannerImage={systemBanners[system]} />
    <section className="bg-sand-100 py-24 md:py-32"><div className="mx-auto max-w-[1200px] px-6"><p className="text-center font-brand text-xs font-bold uppercase tracking-[.18em] text-clay-500">The system</p><h2 className="m-auto text-center mt-4 max-w-[700px] font-brand text-[clamp(2.3rem,4vw,4rem)] font-black uppercase leading-[.93] tracking-[-.055em]">Everything included.</h2><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{content.included.map(item => <div key={item} className="rounded-2xl bg-sand-50 p-6 flex items-center justify-center"><Image src={systemCheckIcons[system]} alt="" width={22} height={22} className="h-[22px] w-auto shrink-0" aria-hidden="true" /><p className="pl-2 font-brand text-sm font-extrabold uppercase leading-snug">{item}</p></div>)}</div></div></section>
    <section className="py-24"><div className="mx-auto grid max-w-[1200px] gap-7 px-6 lg:grid-cols-2"><div className="rounded-3xl border-2 border-sand-200 p-8"><FiSettings className="text-clay-500" size={28} aria-hidden="true" /><h2 className="mt-6 font-brand text-2xl font-black uppercase tracking-[-.04em]">What you can add later.</h2><p className="mt-4 leading-[1.7] text-sand-700">{content.addOns}</p>{system !== "foundation" ? <Link href="/contact" className="mt-7 inline-flex font-brand text-sm font-bold uppercase tracking-[.05em] underline decoration-clay-500 decoration-2 underline-offset-4">Talk to us about custom work →</Link> : null}</div><div className="rounded-3xl bg-sand-950 p-8 text-sand-50"><FiClock className="text-clay-500" size={28} aria-hidden="true" /><h2 className="mt-6 font-brand text-2xl font-black uppercase tracking-[-.04em]">Live in 4 to 8 weeks.</h2><p className="mt-4 leading-[1.7] text-sand-50/70">We start with discovery, create your written gameplan, set up the system, connect what needs connecting, and build the workflows. One point of contact through the whole process.</p></div></div></section>
    <section className="bg-sand-100 py-24"><div className="mx-auto max-w-[900px] px-6"><p className="font-brand text-xs font-bold uppercase tracking-[.18em] text-clay-500">Questions about {content.name}</p><h2 className="mt-4 font-brand text-[clamp(2.2rem,4vw,3.6rem)] font-black uppercase leading-[.94] tracking-[-.055em]">A few straight answers.</h2><div className="mt-10 border-y-2 border-sand-950">{content.faq.map(([question, answer]) => <details key={question} className="group border-b-2 border-sand-950 last:border-0"><summary className="flex cursor-pointer list-none justify-between gap-6 py-6 font-brand text-[15px] font-bold uppercase"><span>{question}</span><span className="text-2xl text-clay-500 group-open:rotate-45">+</span></summary><p className="max-w-[720px] pb-7 leading-[1.7] text-sand-700">{answer}</p></details>)}</div></div></section>
    <section className="bg-sand-900 py-24 text-sand-50"><div className="mx-auto max-w-[900px] px-6 text-center"><FiMapPin className="mx-auto text-clay-500" size={28} aria-hidden="true" /><h2 className="mt-5 font-brand text-[clamp(2.3rem,5vw,4.5rem)] font-black uppercase leading-[.93] tracking-[-.055em]">Ready to talk through {content.name}?</h2><p className="mx-auto mt-6 max-w-[600px] leading-[1.7] text-sand-50/70">Book a free 30-minute consult. We will tell you plainly whether this system fits.</p><div className="mt-9 flex flex-wrap justify-center gap-5"><a href={CALENDLY_URL} className="inline-flex min-h-[50px] items-center gap-2 rounded-full bg-clay-500 px-6 py-3 text-sm font-bold uppercase tracking-[.06em] text-sand-50 no-underline">Book Free Consult <FiArrowRight aria-hidden="true" /></a><Link href="/pricing" className="inline-flex min-h-[50px] items-center rounded-full border-2 border-sand-50/60 px-6 py-3 text-sm font-bold uppercase tracking-[.06em] text-sand-50 no-underline">View comparison</Link></div></div></section>
  </main>;
}
