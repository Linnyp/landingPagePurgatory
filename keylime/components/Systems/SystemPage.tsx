...
const systemContent = {
  foundation: {
    name: "Foundation", price: "$99", heading: "The marketing basics, in one working system.",
  ...
  growth: {
    name: "Growth", price: "$195", heading: "The lead engine for businesses ready for more work.",
  ...
  expansion: {
    name: "Expansion", price: "$495", heading: "The full customer lifecycle, run for you.",
...
const systemBanners: Record<SystemKey, string> = {
  foundation: "/keylimerow.webp",
  growth: "/keylimehalfrow.webp",
  expansion: "/keylimefullrow.webp",
};

/* Plain-English walkthrough of how the tier actually runs, shown under the
   "Everything included." heading. Each one picks up where the tier below it
   left off, mirroring how the `included` lists build on each other. */
const systemOverviews: Record<SystemKey, string> = {
  foundation:
    "Foundation works as one loop. Your one-page website gives people searching for you somewhere credible to land, and every way they reach out — the dedicated business number, texts, and form fills — arrives in a single inbox instead of a personal phone. Each new contact becomes a record in the CRM, so you can see who asked for what and where they stand. Baseline follow-up workflows reply while you are still on a job, and once the work is done, review requests go out on their own to keep the Google listing earning the next call. The practical benefits: nothing gets lost, follow-up stops depending on your memory, and reviews accumulate without you chasing them — for one monthly price, month-to-month, with no stack of subscriptions to manage.",
  growth:
    "Growth takes the Foundation loop and closes the gap where most jobs are lost: the minutes after someone reaches out. A missed call texts the caller back on its own, so the lead that would have gone to the next company on the list gets an answer while you are still on the ladder. A 3–5 page website gives each service its own page to be found on, chatbots handle the questions that come in after hours, and online booking lets people put themselves on the calendar instead of waiting for a callback. Appointment reminders cut the no-shows, birthday outreach keeps you in front of past customers, and SEO Foundation — keyword mapping, citations, technical fundamentals, and your Google Business Profile — puts the whole thing in front of local searches. The practical benefits: faster response times, more booked jobs from the same number of leads, and a local search presence that keeps working between calls.",
  expansion:
    "Expansion runs the Growth engine across the full customer lifecycle, not just the first contact. A custom 5–10 page website and a new article every month give your business real depth for local search, and managed Google Business Profile work keeps the listing current. Leads that are not ready yet drop into nurture sequences that follow up for weeks instead of days. After the job, rewards and referrals turn good customers into repeat work and word of mouth, and a quarterly database reactivation goes back through everyone you have ever served to surface the ones ready to book again. Ongoing platform management means the workflows, integrations, and content stay maintained as the business changes. The practical benefits: more visibility in your market, revenue from customers you already have, and one managed system instead of a team you have to hire.",
};

const systemCheckIcons: Record<SystemKey, string> = {
  foundation: "/keylimewedge.webp",
  growth: "/keylimehalf.webp",
  expansion: "/keylime.webp",
};

export function SystemPage({ system }: { system: SystemKey }) {
  const content = systemContent[system];
  return <main className="pt-8 overflow-hidden bg-sand-50 text-sand-950">
    {/* pt-28 on mobile clears the fixed nav: it sits 2em down and is 85px
        tall, so its bottom edge lands at 117px — py-20 plus the main's pt-8
        put the h1 at 112px, right underneath it. */}
    <section className="relative isolate overflow-hidden pt-28 pb-20 md:py-28"><ParallaxBackground image="/background/keylimeLong.webp" /><HeroGlow className="-left-32 -top-28 h-[460px] w-[460px]" /><HeroGlow className="-right-24 -bottom-32 h-[560px] w-[560px]" /><div className="relative z-10 mx-auto grid max-w-[1200px] gap-10 px-6 lg:grid-cols-[7fr_5fr] lg:items-end"><div><h1 className="font-brand text-[clamp(3rem,6vw,5.7rem)] font-black uppercase leading-[.9] tracking-[-.06em]">{content.heading}</h1><p className="mt-7 max-w-[650px] text-[17px] leading-[1.7] text-sand-700">{content.intro}</p></div><div className="rounded-3xl bg-sand-950 p-8 text-sand-50 shadow-[10px_10px_0_0_#c2552d]"><p className="font-brand text-sm font-bold uppercase tracking-[.12em] text-lime-600">{content.name}</p><p className="mt-3 font-brand text-6xl font-black tracking-[-.06em]">{content.price}<span className="ml-1.5 text-xl tracking-normal text-sand-50/65">/mo</span></p><p className="mt-3 text-sm leading-relaxed text-sand-50/65">$200 one-time setup · Month-to-month</p><a href={CALENDLY_URL} className="mt-7 inline-flex min-h-[50px] w-full items-center justify-center gap-2 rounded-full bg-lime-500 px-6 py-3 text-sm font-bold uppercase tracking-[.06em] text-sand-950 no-underline hover:bg-lime-600">Book Free Consult <FiArrowRight aria-hidden="true" /></a></div></div></section>
    ...
    {/* Centred stack on mobile. From lg it's a two-column, two-row grid: the
        heading owns row 1 in the right column, and the paragraph and card grid
        share row 2 — so the cards start level with the paragraph's top edge
        however tall the heading wraps. Spacing rides on the row gap there and
        on the children's margins on mobile. */}
    <section className="bg-sand-100 py-24 md:py-32"><div className="mx-auto grid max-w-[1200px] gap-0 px-6 lg:grid-cols-[7fr_5fr] lg:items-start lg:gap-x-16 lg:gap-y-7"><div className="text-center lg:col-start-2 lg:row-start-1 lg:text-left"><p className="font-brand text-xs font-bold uppercase tracking-[.18em] text-lime-600">The system</p><h2 className="m-auto mt-4 max-w-[700px] font-brand text-[clamp(2.3rem,4vw,4rem)] font-black uppercase leading-[.93] tracking-[-.055em] lg:m-0">Everything included.</h2></div><p className="m-auto mt-7 max-w-[760px] text-center text-[17px] leading-[1.75] text-sand-700 lg:col-start-2 lg:row-start-2 lg:m-0 lg:text-left">{systemOverviews[system]}</p><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:col-start-1 lg:row-start-2 lg:mt-0">{content.included.map(item => <div key={item} className="rounded-2xl bg-sand-50 p-6 flex items-center justify-center"><Image src={systemCheckIcons[system]} alt="" width={22} height={22} className="h-[22px] w-auto shrink-0" aria-hidden="true" /><p className="pl-2 font-brand text-sm font-extrabold uppercase leading-snug">{item}</p></div>)}</div></div></section>
